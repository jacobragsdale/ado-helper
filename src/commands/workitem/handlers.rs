...
pub async fn dispatch(args: WorkItemArgs, client: &AdoClient, output: &OutputFormat) -> Result<()> {
    match args.command {
        ...
        WorkItemCommand::Link(a) => link(a, client).await,
        WorkItemCommand::Links(a) => links(a, client, output).await,
        WorkItemCommand::LinkRm(a) => link_rm(a, client).await,
        WorkItemCommand::Attach(a) => attach(a, client).await,
...
async fn create(args: CreateArgs, client: &AdoClient, output: &OutputFormat) -> Result<()> {
    ...
    let path = format!("{project}/_apis/wit/workitems/${type_encoded}?api-version=7.1");

    let title = args.fields.title.as_deref()
...
async fn list(args: ListArgs, client: &AdoClient, output: &OutputFormat) -> Result<()> {
    ...
    if let Some(s) = args.search_body.as_deref() {
        clauses.push(format!("[System.Description] CONTAINS '{}'", escape_wiql(s)));
    }
    ...
    let wiql_path = format!("{project}/_apis/wit/wiql?api-version=7.1");
    let result: WiqlResult = client.post_json(&wiql_path, &json!({ "query": wiql })).await?;
    ...
    for chunk in result.work_items.chunks(200) {
        let ids = chunk.iter().map(|w| w.id.to_string()).collect::<Vec<_>>().join(",");
        let path = format!(
            "_apis/wit/workitems?ids={ids}&fields={fields}&api-version=7.1"
        );
        ...
        OutputFormat::Text => {
            let lines: Vec<String> = items
                ...
                .map(|w| {
                    ...
                    let assignee = w.fields.get("System.AssignedTo")
...
async fn view(args: ViewArgs, client: &AdoClient, output: &OutputFormat) -> Result<()> {
    let path = format!("_apis/wit/workitems/{}?$expand=all&api-version=7.1", args.id);
    ...
    match output {
        ...
        OutputFormat::Text | OutputFormat::Table => {
            ...
            let assignee = wi.fields.get("System.AssignedTo")
                .and_then(|v| v.get("displayName"))
                .and_then(|v| v.as_str())
                .unwrap_or("unassigned");
            ...
            print_optional(&wi.fields, "StoryPts",  "Microsoft.VSTS.Scheduling.StoryPoints");
            print_optional(&wi.fields, "Effort",    "Microsoft.VSTS.Scheduling.Effort");
            print_optional(&wi.fields, "OrigEst",   "Microsoft.VSTS.Scheduling.OriginalEstimate");
            print_optional(&wi.fields, "Remaining", "Microsoft.VSTS.Scheduling.RemainingWork");
            print_optional(&wi.fields, "Completed", "Microsoft.VSTS.Scheduling.CompletedWork");
            ...
            print_optional(&wi.fields, "StartDate", "Microsoft.VSTS.Scheduling.StartDate");
            print_optional(&wi.fields, "TargetDate","Microsoft.VSTS.Scheduling.TargetDate");
            print_optional(&wi.fields, "Reason",    "System.Reason");
            if !wi.relations.is_empty() {
                println!("Relations: {} (use `wi links {}`)", wi.relations.len(), wi.id);
            }
            println!("URL:       {}/{}/_workitems/edit/{}", client.org, project, wi.id);
        }
    }
    Ok(())
}
...
async fn update(args: UpdateArgs, client: &AdoClient, output: &OutputFormat) -> Result<()> {
    ...
    for id in &args.ids {
        let path = format!("_apis/wit/workitems/{}?api-version=7.1", id);
...
async fn delete(args: DeleteArgs, client: &AdoClient) -> Result<()> {
    let path = if args.destroy {
        format!("_apis/wit/workitems/{}?destroy=true&api-version=7.1", args.id)
    } else {
...
async fn comments(args: CommentsArgs, client: &AdoClient, output: &OutputFormat) -> Result<()> {
    ...
    match output {
        ...
        OutputFormat::Text | OutputFormat::Table => {
            ...
            let list = resp.get("comments").and_then(|v| v.as_array()).unwrap_or(&empty);
            if list.is_empty() {
...
async fn comment_edit(args: CommentEditArgs, client: &AdoClient, output: &OutputFormat) -> Result<()> {
    let project = args.project.as_deref().unwrap_or(&client.project);
    let path = format!(
        "{project}/_apis/wit/workItems/{}/comments/{}?api-version=7.1-preview.4",
        args.id, args.comment_id
    );
    // Comment edits use PATCH with application/json (NOT json-patch+json)
    let resp = client
        .patch(&path)
...
async fn link(args: LinkArgs, client: &AdoClient) -> Result<()> {
    let (rel, url) = if let Some(target) = args.parent {
        ("System.LinkTypes.Hierarchy-Reverse", workitem_url(client, target))
    } else if let Some(target) = args.child {
        ("System.LinkTypes.Hierarchy-Forward", workitem_url(client, target))
    } else if let Some(target) = args.related {
        ("System.LinkTypes.Related", workitem_url(client, target))
    } else if let Some(target) = args.predecessor {
        ("System.LinkTypes.Dependency-Reverse", workitem_url(client, target))
    } else if let Some(target) = args.successor {
        ("System.LinkTypes.Dependency-Forward", workitem_url(client, target))
    } else if let Some(href) = args.hyperlink {
        ("Hyperlink", href)
    } else {
        bail!("link requires one of: --parent, --child, --related, --predecessor, --successor, --hyperlink");
    };
    ...
    let _: WorkItem = client.patch_json_patch(&path, &ops).await?;
    println!("Added {rel} link to #{}", args.id);
    Ok(())
}

async fn links(args: LinksArgs, client: &AdoClient, output: &OutputFormat) -> Result<()> {
    let path = format!("_apis/wit/workitems/{}?$expand=relations&api-version=7.1", args.id);
    let wi: WorkItem = client.get_json(&path).await?;
    ...
    match output {
        ...
        OutputFormat::Text | OutputFormat::Table => {
            ...
            for (i, r) in wi.relations.iter().enumerate() {
                let comment = r.attributes.get("comment").and_then(|v| v.as_str()).unwrap_or("");
                let name = r.attributes.get("name").and_then(|v| v.as_str()).unwrap_or("");
...
async fn link_rm(args: LinkRmArgs, client: &AdoClient) -> Result<()> {
    let path = format!("_apis/wit/workitems/{}?api-version=7.1", args.id);
    let ops = vec![PatchOp {
        op: "remove".into(),
...
async fn attach(args: AttachArgs, client: &AdoClient) -> Result<()> {
...
async fn history(args: HistoryArgs, client: &AdoClient, output: &OutputFormat) -> Result<()> {
    ...
    match output {
        ...
        OutputFormat::Text | OutputFormat::Table => {
            let empty: Vec<serde_json::Value> = Vec::new();
            let updates = resp.get("value").and_then(|v| v.as_array()).unwrap_or(&empty);
            if updates.is_empty() {
                println!("(no history)");
                return Ok(());
            }
...
async fn open(args: OpenArgs, client: &AdoClient) -> Result<()> {
    let project = args.project.as_deref().unwrap_or(&client.project);
    let url = format!("{}/{}/_workitems/edit/{}", client.org, project, args.id);
